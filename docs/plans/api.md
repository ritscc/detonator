...
3. **3.0 秒**後、爆発時点スナップショットで Rooted Prim-MST を再計算し、BFS で連鎖を適用する。
...
| 対象座標省略可 | `mine_remover_*` / `purify`（省略時は Facing から 4 方向補正して算出） |
...
| `cats_eye` | 全未回収 CP を一定時間チーム共有表示する。効果開始/終了は `cats_eye_activated` / `cats_eye_expired` で通知する。 |
...
| `disposable_life` | **手動使用**で一定時間バフを付与し、そのバフ有効中に死亡判定が来た場合だけ**自動消費**して死亡を回避する。優先消費順位は 1 位。 |
...
- 侵食警告時間は**ベース 3 秒**で、侵食頻度に応じて式駆動で短縮される。
- 対象セルは frontline 候補から幅優先で選ばれる。詳細は [Timers and Event Queue > 侵食タイマー](#侵食タイマー) を参照。
...
- `ready` コマンドではなく、**全プレイヤーが各自の初期スポーン地点へ到達した時点**で終了する。
...
- 同 tick に複数エントリがある場合、**順不同**で逐次処理する。各爆発後に盤面を再計算し、後続エントリへ反映する。
- タイブレークは決定的規則を要求するが、**最終規則の詳細は未確定**（GDD §20 参照）。
...
- 侵食インターバルは式駆動。
- **各フェーズで侵食力（変換マス数）を算出**する。
- 警告時間のベースは **3 秒**（式で短縮される）。
- `erosion_warn` 到達時:
  1. frontline 候補（安全領域外周に接する安全マス群）を抽出する。
  2. frontline から**幅優先**で必要マス数ぶん対象化する。
  3. 候補不足時は**深さを進めて**探索を継続する。
  4. `erosion_warning` 送信 + `ErosionState.warningCellKeys` 更新。
- `erosion_convert` 到達時:
  1. `SafeMine:DangerousMine` 比を式から算出する。
  2. 対象セルをその比率でランダム配置しつつ変換する。
  3. 旗 / Relay Point / 地上アイテムを除去する。
  4. 対象セル上プレイヤーを即死させる。
  5. 侵食後に周囲安全マスの `adjacentMineCount` を再計算する。
  6. 次フェーズの `erosion_warn` をキューに追加する。
- **スポーン地点セルも侵食対象になりうる**。
- `bridge` で Safe 化した元 Hole セルも侵食対象になりうる。
