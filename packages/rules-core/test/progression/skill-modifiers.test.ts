...
import { SkillType, type ItemType } from "@detonator/protocol";
...
describe("aggregateSkillModifiers", () => {
	...
	it("maps all skill definitions into the expected aggregated fields", async () => {
		...
		expect(aggregateSkillModifiers(stacks)).toEqual({
			movementSpeedBoostRatio:
				skills[SkillType.MovementSpeedBoost].valueRoll.max / 100,
			detonateCooldownReductionSec:
				skills[SkillType.DetonateCooldownReduction].valueRoll.max,
			expGainBoostRatio: skills[SkillType.ExpGainBoost].valueRoll.max / 100,
			comboMultiplierBonus:
				skills[SkillType.ComboMultiplierBoost].valueRoll.max,
			erosionCooldownIncreaseRatio:
				skills[SkillType.ErosionCooldownIncrease].valueRoll.max / 100,
			itemDropRateBoostRatio:
				skills[SkillType.ItemDropRateBoost].valueRoll.max / 100,
			itemPickupRangeBoostCells:
				skills[SkillType.ItemPickupRangeBoost].valueRoll.max,
			itemSlotIncreaseCount: skills[SkillType.ItemSlotIncrease].valueRoll.max,
			cpDetectionRangeBoostCells:
				skills[SkillType.CpDetectionRangeBoost].valueRoll.max,
			erosionForewarningSec:
				skills[SkillType.ErosionForewarning].valueRoll.max,
			deathItemKeepChanceRatio:
				skills[SkillType.DeathItemKeepChance].valueRoll.max / 100,
			wastelandPenaltyReductionRatio:
				skills[SkillType.WastelandSpeedReduction].valueRoll.max / 100,
			respawnReductionSec:
				skills[SkillType.RespawnTimeReduction].valueRoll.max,
			chordOwned: true,
		});
	});
});
