...
export function selectFrontlineTargets(input: {
  ...
  while (
    ...
    const component = collectConnectedComponent(seed, currentFrontlineKeys, input.grid);
    ...
    currentFrontline = buildAdvancedFrontline(input.grid, selectedKeys, excludedKeys);
  }
...
function compareCoordsWithinColumn(left: GridCoord, right: GridCoord, seed: GridCoord): number {
  ...
  return (
    Math.abs(left.y - seed.y) - Math.abs(right.y - seed.y) || left.y - right.y || left.x - right.x
  );
}
