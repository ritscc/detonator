...
import type { ErosionState } from "../ErosionState.js";
...
export function clearAllErosionWarnings(grid: GridState, erosionState?: ErosionState): void {
  ...
  if (erosionState !== undefined) {
    erosionState.warningCellKeys.clear();
  }
}

export function convertAllMineCellsToSafe(grid: GridState): void {
  for (const cell of grid.cells) {
    if (cell.cellType === CellType.SafeMine || cell.cellType === CellType.DangerousMine) {
      cell.cellType = CellType.Safe;
    }
  }
}

export function resetPlayersForNewFloor(players: MapSchema<PlayerState>): void {
