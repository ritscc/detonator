...
import { ErosionState } from "../src/ErosionState.js";
import { GridState } from "../src/GridState.js";
import type { GroundItemState } from "../src/GroundItemState.js";
...
describe("schema utility helpers", () => {
  ...
  it("clears erosionWarning on every cell and empties warningCellKeys when erosionState is provided", () => {
    const grid = createGrid([CellType.Safe, CellType.SafeMine]);
    grid.cells[0]!.erosionWarning = true;
    grid.cells[1]!.erosionWarning = true;

    const erosionState = new ErosionState();
    erosionState.warningCellKeys.push("0,0");
    erosionState.warningCellKeys.push("1,0");

    clearAllErosionWarnings(grid, erosionState);

    expect(grid.cells[0]!.erosionWarning).toBe(false);
    expect(grid.cells[1]!.erosionWarning).toBe(false);
    expect([...erosionState.warningCellKeys]).toEqual([]);
  });

  it("applies the grid reset helpers that clear transient marks and convert mine cells", () => {
    ...
    expect(grid.cells.map((cell) => cell.flagged)).toEqual([false, false, false, false]);
    expect(grid.cells.map((cell) => cell.hasRelayPoint)).toEqual([false, false, false, false]);
    expect(grid.cells.map((cell) => cell.erosionWarning)).toEqual([false, false, false, false]);
  ...
  it("resetPlayersForNewFloor resets floor-specific state but preserves cumulative progression and identity fields", () => {
    const players = new MapSchema<PlayerState>();
