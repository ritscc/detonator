...
export async function loadConfig(input: LoadConfigInput): Promise<DeepReadonly<SharedGameConfig>> {
...
function mergeDeep<T>(base: T, override: unknown): T {
  ...
  for (const key of keys) {
    ...
    merged[key] =
      overrideValue === undefined ? cloneDeep(baseValue) : mergeDeep(baseValue, overrideValue);
  }
